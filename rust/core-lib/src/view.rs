...
impl View {
    ...
    fn unit(&self, text: &Rope, offset: usize, granularity: SelectionGranularity) -> Range<usize> {
        match granularity {
    ...
    pub fn render_styles(
        ...
        let style_spans = style_spans.subseq(start..end);
        ...
        for span in style_spans.iter() {
            let style_id = self.get_or_def_style_id(client, styles, &span.data);
            rendered_styles.push((span.iv.start as isize) - ix);
            rendered_styles.push(span.iv.end as isize - span.iv.start as isize);
            rendered_styles.push(style_id as isize);
            ix = span.iv.end as isize;
        }
        rendered_styles
    }
    ...
    fn interval_of_visible_region(&self, text: &Rope) -> Range<usize> {
