...
mod tests {
    ...
    fn make_both(sizes: &[usize], first_one: bool, exact_len: Option<usize>) -> (subset::Subset, Subset) {
        let mut mb = SubsetBuilder::new();
        let mut sb = subset::SubsetBuilder::new();
        let mut count_one = first_one;
        let mut cur_pos = 0;
        for len in sizes.iter().cloned() {
            let trimmed_len = exact_len.map(|l| cmp::min(l-cur_pos, len)).unwrap_or(len);
            if trimmed_len == 0 { continue }
            let count = if count_one { 1 } else { 0 };
            mb.push_segment(trimmed_len, count);
            if count_one {
                sb.add_range(cur_pos, cur_pos + trimmed_len);
            }
            cur_pos += trimmed_len;
            count_one = !count_one;
        }
    ...
    fn check_same_deletion(total_len: usize, s: &subset::Subset, m: &Subset) -> bool {
        let mut test_str = String::new();
        for i in 0..total_len {
            let c: char = char::from((' ' as u8) + ((i % 94) as u8));
            test_str.push(c);
        }

        s.delete_from_string(&test_str) == m.delete_from_string(&test_str)
    }

    quickcheck! {
        fn p_same_deletion(sizes: Vec<usize>, first_one: bool) -> bool {
            let (s, m) = make_both(&sizes, first_one, None);
            check_same_deletion(sizes.iter().sum(), &s, &m)
        }

        fn p_same_exact_len_deletions(sizes: Vec<usize>, first_one: bool) -> bool {
            let (s, m) = make_both(&sizes, first_one, Some(100));
            check_same_deletion(100, &s, &m)
        }

        fn p_same_complement(sizes: Vec<usize>, first_one: bool) -> bool {
            let (s, m) = make_both(&sizes, first_one, None);
            let total_len = sizes.iter().sum();
            let sc = s.complement(total_len);
            let mc = m.complement();
            check_same_deletion(total_len, &sc, &mc)
        }

        fn p_same_trans_expand(sizes: Vec<usize>, first_one: bool, sizes2: Vec<usize>, first_one2: bool) -> bool {
            let (s, m) = make_both(&sizes, first_one, None); // transform
            let prev_len = m.count(CountMatcher::Zero);
            let (s2, m2) = make_both(&sizes2, first_one2, Some(prev_len));

            let s3 = s2.transform_expand(&s);
            let m3 = m2.transform_expand(&m);

            check_same_deletion(sizes.iter().sum(), &s3, &m3)
        }

        fn p_same_trans_union(sizes: Vec<usize>, first_one: bool, sizes2: Vec<usize>, first_one2: bool) -> bool {
            let (s, m) = make_both(&sizes, first_one, None); // transform
            let prev_len = m.count(CountMatcher::Zero);
            let (s2, m2) = make_both(&sizes2, first_one2, Some(prev_len));

            let s3 = s2.transform_union(&s);
            let m3 = m2.transform_union(&m);

            check_same_deletion(sizes.iter().sum(), &s3, &m3)
        }

        fn p_same_union(sizes: Vec<usize>, first_one: bool, sizes2: Vec<usize>, first_one2: bool) -> bool {
            let total_len = sizes.iter().sum();
            let (s, m) = make_both(&sizes, first_one, None); // transform
            ...
            check_same_deletion(total_len, &s3, &m3)
        }

        fn p_same_trans_shrink(sizes: Vec<usize>, first_one: bool, sizes2: Vec<usize>, first_one2: bool) -> bool {
            let total_len = sizes.iter().sum();
            let (s, m) = make_both(&sizes, first_one, None); // transform
            let (s2, m2) = make_both(&sizes2, first_one2, Some(total_len));
        ...
        fn p_same_mapping(sizes: Vec<usize>, first_one: bool, map_gaps: Vec<usize>) -> bool {
            let (s, m) = make_both(&sizes, first_one, None);
            let mut s_map = s.mapper();
            let mut m_map = m.mapper(CountMatcher::NonZero);

            // turn into something in increasing order
            let mut to_map = vec![];
            let mut i: usize = 0;
            for g in map_gaps.iter().cloned() {
                i += g;
                to_map.push(i);
            }

            to_map.iter().cloned().all(|i| {
                s_map.doc_index_to_subset(i) == m_map.doc_index_to_subset(i)
            })
        }
    }
}
