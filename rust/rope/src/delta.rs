...
impl<N: NodeInfo> Delta<N> {
    ...
    pub fn apply(&self, base: &Node<N>) -> Node<N> {
        ...
        for elem in &self.els {
            match *elem {
                DeltaElement::Copy(beg, end) => base.push_subseq(&mut b, Interval::new_closed_open(beg, end)),
...
impl<N: NodeInfo> InsertDelta<N> {
    ...
    pub fn transform_shrink(&self, xform: &Subset) -> InsertDelta<N> {
        ...
        let els = self.0
            ...
            .iter()
            .map(|elem| match *elem {
                DeltaElement::Copy(b, e) => DeltaElement::Copy(m.doc_index_to_subset(b), m.doc_index_to_subset(e)),
...
mod tests {
    ...
    fn is_simple_delete() {
        ...
        let d = Delta::simple_edit(Interval::new_closed_open(10, 11), Rope::from(""), TEST_STR.len());
